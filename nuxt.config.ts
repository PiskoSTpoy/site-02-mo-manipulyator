...
if (document.readyState === 'complete') { __ymBoot(); } else { window.addEventListener('load', __ymBoot); setTimeout(__ymBoot, 4000); }`,
      ...
      link: [
        // Волна приёмки (29.08.2026). Файл favicon.ico лежал в public/ с самого
        // начала, но ссылки на него в <head> не было ни разу — Nuxt не добавляет
        // её автоматически. Яндекс.Вебмастер не смог найти фавиконку именно
        // поэтому: браузер и робот узнают о ней только через явный <link rel="icon">.
        // Волна приёмки (31.08.2026). После этого Вебмастер нашёл .ico, но
        // указал на его размер: 32×32 меньше требуемых 120×120. Вместо
        // растровой версии — SVG (favicon.svg), тот же мотив, что и в
        // brand__mark шапки: масштабируется без потери качества на любом
        // размере, современные браузеры используют его первым. .ico остаётся
        // как fallback для тех, кто SVG-фавиконки не читает.
        // 07.09.2026: диагностика Вебмастера (BIG_FAVICON_ABSENT) продолжала
        // гореть спустя неделю после SVG-фикса — похоже, крауler Яндекса для
        // этой конкретной проверки не растеризует SVG и ищет именно растровую
        // иконку крупного размера. Добавлена растровая PNG 120×120 (тот же
        // SVG, отрендерен через sharp), плюс apple-touch-icon 180×180 — тоже
        // рекомендуемый размер для «крупной» иконки, бесплатно раз уже
        // рендерили. SVG и .ico остаются первыми в списке (актуальны для
        // браузеров), PNG — специально под этот класс проверок.
